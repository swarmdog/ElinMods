...
async def test_order_expire(client, time_machine):
    ...
    # Create order with 1-second deadline
    # Advance time
...
async def test_rep_gain_on_success(client):
    ...
    # Submit shipment with good quality → check player rep increased

async def test_rep_loss_on_failure(client):
    """Failed shipment → negative rep delta."""
    # Submit shipment with terrible quality → check player rep decreased

async def test_rank_promotion(client):
    """Accumulate enough rep → rank upgrades."""
    # Submit many successful shipments → verify rank changes
```

### Satisfaction Algorithm Tests (Unit tests)
...
| Test | Steps | Expected |
|------|-------|----------|
| Orders display | Open Network Panel → Market tab | Available orders listed with type, quantity, potency, payout |
